...
- `add_fpsim()` makes use of newer measures of dyadic foreign policy similarity generated by `{fpsim}`. 
- `download_extdata()` has a few changes, including arguments of `warning` and `confirm`. Check function documentation for more.
