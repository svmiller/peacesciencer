...
[^whittle]: The "whittle" (`wc_`) class of functions are a suite of functions that whittle dyadic dispute-years to true dyad-year data. They are effectively used in `add_cow_mids()` and `add_gml_mids()` for dyad-year data, but are offered here to allow users to employ their own case exclusion rules to create their own dyad-year conflict data. A vignette on `{peacesciencer}`'s website explains these case exclusion rules in some detail.
